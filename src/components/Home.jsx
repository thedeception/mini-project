...
export default function Home({setSelectedRecipe}) {
  ...
  const nav=useNavigate()
    ...
    const getRecipes = async (FROM, TO) => {
      try {
        ...
        const responseData = await axios.get(`https://api.edamam.com/search?${searchParams.toString()}`).then(res=>{
            setIsLoading(false)
 ...
 <header className="container m-auto pb-28">
        <Navbar from={from} to={to} totalRecipes={!isLoading && response.count} />
      </header>
      ...
      <form className="container m-auto flex justify-center gap-1 items-center pb-6 " onSubmit={handleSubmit}>
        ...
        <span className='flex bg-primary-content'>({!isLoading &&response&& response.count} Recipes)</span>
      </form>
      ...
      <div className="container flex gap-11 justify-center flex-wrap m-auto">
        {recipe.map((recipe) => (
          ...
          <RecipeCard onClick={()=>{setSelectedRecipe(recipe.recipe);
         return  nav(`${recipe.recipe.label}`);
        }}
      ...
      <button className="btn w-44 text-slate-950 btn-outline my-6" disabled={!isLoading && !response.more} onClick={handleLoadMore}>
        Load More
      </button>
        </>
    )
